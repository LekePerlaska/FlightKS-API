...
<@layout.registrationLayout displayMessage=!messagesPerField.existsError('username','password') displayInfo=realm.password; section>
    ...
    <#elseif section = "info" >
        <#-- FlightKS registration is handled by the frontend, so link there instead
             of Keycloak's built-in registration (which is disabled on this realm). -->
        <#if realm.password>
            <div id="kc-registration-container">
                <div id="kc-registration">
                    <span>${msg("noAccount")} <a tabindex="8"
                                                 href="${properties.flightksRegisterUrl!'/auth/register'}">${msg("doRegister")}</a></span>
                </div>
            </div>
        </#if>
